...
{
    public partial class FrmBSpline : FrmBase
    {
        List<Point> points = new List<Point>();
        private List<Point> bsplinesCurvePoints = new List<Point>();
        private Point? currentPositionOnCurve = null;
        public FrmBSpline() : base(Constants.BSplinesGuidance, "BSplines Curves")
        {
            InitializeComponent();
        ...
        private void canvas_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                this.txtGrade.Text = (points.Count + 1).ToString();
            ...
            else if (e.Button == MouseButtons.Right)
            {
                ...
                {
                    bsplinesCurvePoints = BSplineCurve.GenerateCurvePoints(points, 1000);

                    if (bsplinesCurvePoints.Count > 0)
                    {
                        tbCurve.Value = 0;
                        currentPositionOnCurve = bsplinesCurvePoints[0];
                    }
                    else
                    {
                        currentPositionOnCurve = null;
                    }

                    RenderToCanvasImage();
                }else
                {
                    MessageBox.Show("At least 4 points are required to generate a B-Spline curve.", "Insufficient Points", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
        ...
        private void canvas_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (bsplinesCurvePoints.Count > 1)
            {
                using (Pen pen = new Pen(Config.Instance.LineColor, 2))
                {
                    for (int i = 0; i < bsplinesCurvePoints.Count - 1; i++)
                    {
                        g.DrawLine(pen, bsplinesCurvePoints[i], bsplinesCurvePoints[i + 1]);
                    }
                }
            }
        ...
        private void tbCurve_Scroll(object sender, EventArgs e)
        {
            if (bsplinesCurvePoints.Count > 0)
            {
                ...
                if (index >= 0 && index < bsplinesCurvePoints.Count)
                {
                    currentPositionOnCurve = bsplinesCurvePoints[index];
                    RenderToCanvasImage();
                }
            }
        }
